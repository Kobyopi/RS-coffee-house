import{i as p,a as g,b as v,u as y,S as o,f as d,c as l}from"./common-0sy5bLwL.js";import{a as f}from"./api-Cjz2_jBT.js";class C{cartItemsContainer;cartSummaryContainer;constructor(){this.cartItemsContainer=document.getElementById("cartItems"),this.cartSummaryContainer=document.getElementById("cartSummary"),this.init()}init(){this.initBurgerMenu(),this.renderCart(),y()}initBurgerMenu(){const t=document.querySelector(".burger-icon"),e=document.querySelector(".burger-menu"),a=document.querySelectorAll(".burger-link");t&&t.addEventListener("click",()=>{t.classList.toggle("active"),e.classList.toggle("active"),document.body.classList.toggle("burger-open")}),a&&a.forEach(s=>{s.addEventListener("click",()=>{t.classList.remove("active"),e.classList.remove("active"),document.body.classList.remove("burger-open")})})}renderCart(){const t=o.getCart(),e=o.isUserLoggedIn(),a=o.getUser();if(!this.cartItemsContainer||!this.cartSummaryContainer)return;if(t.items.length===0){this.renderEmptyCart();return}this.cartItemsContainer.innerHTML="",t.items.forEach((n,i)=>{const c=this.createCartItemElement(n,i);this.cartItemsContainer.appendChild(c)}),this.cartSummaryContainer.innerHTML="";const s=document.createElement("div");s.className="cart-summary-content";let r=t.totalPrice;e&&t.totalDiscount>0&&(r=t.totalPrice-t.totalDiscount),e?(s.innerHTML=`
    ...
    `,this.cartSummaryContainer.innerHTML="")}removeItem(t){o.removeFromCart(t),this.renderCart(),d(o.getCartItemCount()),l("Item removed from cart","success")}async confirmOrder(){const t=o.getCart();if(t.items.length===0)return;const e=document.getElementById("confirmOrderBtn");if(!e)return;e.disabled=!0,e.innerHTML=`
      <div class="loader-spinner loader-spinner-small"></div>
      Processing...
    `;const a=new Map;t.items.forEach(r=>{const n=`${r.productId}-${r.size}-${r.additives.sort().join(",")}`,i=a.get(n);i?i.quantity+=1:a.set(n,{productId:r.productId,size:r.size.toLowerCase(),additives:r.additives,quantity:1})});const s={items:Array.from(a.values()),totalPrice:t.totalPrice-t.totalDiscount};try{const r=await f.placeOrder(s);r.error?(l(r.error||"Something went wrong. Please, try again","error"),e.disabled=!1,e.textContent="Confirm Order"):(o.clearCart(),this.cartItemsContainer&&(this.cartItemsContainer.innerHTML=`
          ...
          `),this.cartSummaryContainer&&(this.cartSummaryContainer.innerHTML=""),d(0))}catch{l("Something went wrong. Please, try again","error"),e.disabled=!1,e.textContent="Confirm Order"}}}document.addEventListener("DOMContentLoaded",()=>{p(),new C,g(),v()});
